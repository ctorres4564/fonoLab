...
const modulo = unit.modules.find((m) => m.slug === 'principios-intervencao-fonologica')!
...
// ─────────────────────────────────────────────────────────────────────────────
// Módulo 3 — Princípios de intervenção fonológica (conteúdo real, status "published")
// ─────────────────────────────────────────────────────────────────────────────
describe('Módulo 3 — Princípios de intervenção fonológica: carregamento', () => {
  it('carrega o módulo com id e slug estáveis', () => {
    expect(modulo).toBeDefined()
    expect(modulo.id).toBe('mod-principios-intervencao-fonologica')
    expect(modulo.slug).toBe('principios-intervencao-fonologica')
    expect(modulo.order).toBe(3)
  })
...
describe('Módulo 3 — conteúdo de Aprender', () => {
  ...
  it('não prescreve nenhum modelo de intervenção como universalmente superior', () => {
    const summaryText = modulo.learn.summary.map((p) => p.text).join(' ').toLowerCase()
    expect(summaryText).toContain('universalmente superior')
  })
})
...
describe('Módulo 3 — conteúdo de Aplicar', () => {
  ...
  it('tem 1 caso fictício com disclaimer explícito e sem plano terapêutico completo', () => {
  ...
  it('as questões do caso fictício não indicam um único modelo como obrigatório/definitivo', () => {
    const case1 = modulo.apply.fictionalCases[0]
    case1.questions.forEach((q) => {
      const correct = q.alternatives.find((a) => a.id === q.correctAlternativeId)!
      expect(correct.text.toLowerCase()).not.toContain('obrigatoriamente')
    })
  })
})
...
describe('Módulo 3 — validação Zod e preservação dos demais módulos', () => {
  ...
  it('os Módulos 1 e 2 permanecem publicados e intactos', () => {
  ...
  it('o Módulo 4 está publicado, com conteúdo real', () => {
    const modulo4 = unit.modules.find((m) => m.slug === 'planejamento-terapeutico-transtornos-fonologicos')!
