...
const modulo = unit.modules.find((m) => m.slug === 'afasias-neurolinguistica-adulto')!
...
// ─────────────────────────────────────────────────────────────────────────────
// Módulo — Afasias e Neurolinguística no Adulto e Idoso (status "published")
// ─────────────────────────────────────────────────────────────────────────────
describe('Módulo Afasias: carregamento', () => {
...
describe('Módulo Afasias — ausência de placeholders', () => {
...
describe('Módulo Afasias — conteúdo de Aprender', () => {
  ...
  it('tem entre 12 e 16 conceitos essenciais, todos com termo, definição e exemplo', () => {
    expect(modulo.learn.essentialConcepts.length).toBeGreaterThanOrEqual(12)
    expect(modulo.learn.essentialConcepts.length).toBeLessThanOrEqual(16)
  ...
  it('tem entre 7 e 10 evidências, todas com fonte e implicação clínica', () => {
    expect(modulo.learn.evidence.length).toBeGreaterThanOrEqual(7)
  ...
  it('tem ao menos 8 limitações/cuidados registrados', () => {
    expect(modulo.learn.limitations.length).toBeGreaterThanOrEqual(8)
  })
  ...
  it('glossário tem entre 14 e 18 termos, todos com definição e exemplo', () => {
    expect(modulo.learn.glossary.length).toBeGreaterThanOrEqual(14)
  ...
  it('reconhece explicitamente os limites da classificação clássica das afasias', () => {
    const summaryText = modulo.learn.summary.map((p) => p.text).join(' ').toLowerCase()
    expect(summaryText).toContain('perfis mistos')
  })

  it('não trata nenhuma abordagem de reabilitação como protocolo universal', () => {
    const summaryText = modulo.learn.summary.map((p) => p.text).join(' ').toLowerCase()
    expect(summaryText).toContain('protocolo universal')
  })
})

describe('Módulo Afasias — conteúdo de Aplicar', () => {
  ...
  it('as alternativas corretas do caso fictício usam formulações cautelosas quanto ao diagnóstico', () => {
    const case1 = modulo.apply.fictionalCases[0]
    case1.questions.forEach((q) => {
      const correct = q.alternatives.find((a) => a.id === q.correctAlternativeId)!
      expect(correct.text.toLowerCase()).not.toContain('certamente')
    })
  })
})

describe('Módulo Afasias — conteúdo de Revisar', () => {
  it('tem entre 22 e 28 flashcards, todos completos', () => {
    expect(modulo.review.flashcards.length).toBeGreaterThanOrEqual(22)
    expect(modulo.review.flashcards.length).toBeLessThanOrEqual(28)
...
describe('Módulo Afasias — referências', () => {
...
describe('Módulo Afasias — validação Zod e preservação dos demais módulos da unidade', () => {
  it('passa integralmente na validação do schema, já com status "published"', () => {
    const result = validateModule(modulo)
    if (!result.success) {
      console.error('Erros de validação do Módulo Afasias:', result.errors)
    }
